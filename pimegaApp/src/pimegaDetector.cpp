...
Codec_t pimegaDetector::getCodec() {
  compression_codec_t comp_codec = GetCompressionCodec(pimega);
  compression_compressor_t comp_compressor = GetCompressionCompressor(pimega);
  compression_shuffle_t comp_shuffle = GetCompressionShuffle(pimega);
...
void pimegaDetector::updateEpicsFrame(void *data, size_t size,
  ...
  int status = NDArray::computeArrayInfo(2, array_dims, ndarray_dtype, &info);
  if (status != ND_SUCCESS) {
    updateIOCStatus("Failed to compute array info");
    return;
  }

  NDArray *PimegaNDArray = this->pNDArrayPool->alloc(
      2, array_dims, ndarray_dtype, info.totalBytes + BLOSC_MAX_OVERHEAD, NULL);
...
void pimegaDetector::updateIOCStatus(const std::string_view message) {
  epicsInt8 *array = (epicsInt8 *)message.data();
  doCallbacksInt8Array(array, message.length(), PimegaIOCStatusMessage, 0);
}

void pimegaDetector::updateServerStatus(const std::string_view message) {
  epicsInt8 *array = (epicsInt8 *)message.data();
  doCallbacksInt8Array(array, message.length(), PimegaServerStatusMessage, 0);
}
